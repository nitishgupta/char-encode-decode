...
class PreTrainingDecoderModel(Model):
  ...
  def __init__(self, num_layers, batch_size, h_dim, dec_input_batch,
    ...
    # This size should be equal to the encoder h_dim
    self.h_dim = h_dim
    ...
    with tf.variable_scope(scope_name) as scope:
      decoder_cell = tf.nn.rnn_cell.BasicLSTMCell(self.h_dim,
                                                  state_is_tuple=True)
      decoder_dropout_cell = tf.nn.rnn_cell.DropoutWrapper(cell=decoder_cell,
  ...
  def append_encoder_last_output(self, encoder_last_output):
    ...
    time_list_dec_input = tf.map_fn(
      lambda x : tf.concat(concat_dim=1, values=[x, encoder_last_output]),
